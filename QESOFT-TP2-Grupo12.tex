...
Relativamente à análise do produto, com base no conhecimento que o grupo possui à cerca da equipa e do projeto em questão, este acredita que pode ser possível relacionar a natureza e particularidades de algumas das regras violadas com a falta de experiência da equipa de desenvolvido quanto a algumas das particularidades da arquitetura de software, uma vez que foi o maior e mais complexo projeto desenvolvido até ao momento da sua elaboração.\\ 
Na figura ~\ref{figClass} é possível observar o número de violação de cada categoria por classe. Da sua observação é possível concluir que a classe PdfExport é a que possui mais violações. A figura ~\ref{figQuality} apresenta o número de violações por categoria.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\linewidth]{chart_classes.PNG}
	\caption{Número de regras violadas em função da característica de qualidade, por classe \small}
	\label{figClass}
\end{figure}
